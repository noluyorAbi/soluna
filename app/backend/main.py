...
def create_interactive_activity_plot(members_data, donation_weight=1.0, donation_received_weight=0.5,
    ...
    explanation_text = (
        ...
        "<p>Aktivität = (Spenden Gegeben × Gewichtung) + "
        "(Spenden Erhalten × Gewichtung) + "
        "(Gewonnene Angriffe × (Basisgewichtung + Trophäen / Skalierungsfaktor))</p>"
        "<p>Gewichtungsfaktoren:</p>"
        f"<ul>"
        f"<li>Spenden Gegeben: {donation_weight}</li>"
        f"<li>Spenden Erhalten: {donation_received_weight}</li>"
        f"<li>Gewonnene Angriffe: {attack_base_weight} + (Trophäen / {trophy_scale})</li>"
        f"</ul>"
    )
